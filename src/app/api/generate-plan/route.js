...
import { connectDB } from '../../lib/dbConnect'; // Import centralized connection logic
...
export async function POST(req, res) {
  try {
    const { latitude, longitude, placeIds, priceRange,city } = await req.json();
    const startplaceId = await getPlaceIdFromCoordinates(latitude, longitude)
    ...
    const newPlan = new Plan({
      unique_id: uniqueId,
      city:city,
      final_routes: final_optimized_route,
      final_durations: final_adjacent_duration,
    });

    await newPlan.save();
    // console.log(uniqueId);

    return NextResponse.json({ uniqueId: uniqueId });
  } catch (error) {
